...
4. При создании события типа *Рождение* поле Родился должно быть просто текстовым полем.
	 И под этим полем должно быть еще два поисковое поле Родитель1 и Родитель2 второе поле не обязательно
	 где в каждом из поле может быть или отец или мать ребенка.
...
# Что обезательно а что нет
#### Поле подробности не обязательно везде

#### Свадьба:
1. жених и невеста обязательно
2. поле адресс обязательно
3. дата проведения обязательно

#### Похороны
1. все обязательно

3. Рождение
1. родился обязательно
2. один из двух родителей обязательно (но ты мне возвращаешь поля mather father как null если там не че нет) a
   проверку я сам буду делать со своей стороны
3. дата рождения обязательно

#### Cбор средств
1. все обязательно кроме адресс проведения
#### Другое
1. все обязательно кроме адресс проведения

# Как я принимаю модель с сервера
#### ? на конце типа данных означает что это поле может быть null
* [!] обязательно обрати внимание на тип данных где стоит ?

```dart
	final int id;
	/// *action* can be birth, funeral, wedding,
	/// after... other and fundraising
	final String action;
	final String date;
	final DateTime datePub; // в твоем случаи это будет строка присылай как обыную дату и все
	...
	/// can be: father, mather, spouce and other (will be plus)
	final String eventLine;
	final GeneralTeipEntity teip;
	final bool isRemind;
	final bool canSeeComments;
	/// Это поле подробности
	final String? content;
	/// Это поле которое появляется при создании события 
	/// типа сбор или другое (цель, наименование события)
	final String? info;
	final int? price;
	final String? address;
	final String? bornFio;
	final EventUserEntity? organizer;
	final EventUserEntity? dead;
	final EventUserEntity? father;
	final EventUserEntity? mather;
	final EventUserEntity? groom;
	final EventUserEntity? wife;
```
